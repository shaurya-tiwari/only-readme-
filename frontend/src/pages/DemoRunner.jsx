...
import { formatCurrency, formatDateTime, formatPercent, formatRelative } from "../utils/formatters";
...
export default function DemoRunner() {
  ...
  async function handleRun(scenarioId) {
    ...
    try {
      const demoRunId = `${scenarioId}-${Date.now()}`;
      await triggersApi.scenario(scenarioId);
      const response = await triggersApi.check({ city: "delhi", scenario: scenarioId, demo_run_id: demoRunId });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Monitored zones" value={status?.zones_checked ?? 0} hint="Current trigger status call" tone="gold" />
      </div>
      ...
      <div className="panel p-6">
        ...
        <p className="mt-4 text-sm text-ink/60">
          RideShield is not waiting for a worker to file a claim. The scheduler keeps checking signals in the background and the demo runner is an explicit admin override for showing cause and effect on demand.
        </p>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {snapshotSummary.map((snapshot) => (
            <div key={snapshot.zone} className="rounded-2xl bg-black/[0.03] p-4">
              <p className="text-sm font-semibold">{snapshot.zone}</p>
              <p className="mt-2 text-xs uppercase tracking-[0.2em] text-ink/45">Active triggers</p>
              <p className="mt-2 text-sm text-ink/70">{snapshot.triggers_active.length ? snapshot.triggers_active.join(", ") : "None"}</p>
            </div>
          ))}
        </div>
      </div>
